...
export const exec_bridge = async( bridgeTx: BridgeTx ) => {
    ...
    const { signer, fromToken, fromChain, toChain, amount, routeTx } = bridgeTx
    ...
    try {
        ...
        const tx = await signer.sendTransaction( { to: txTarget, data: txData, value: value, nonce: nonce, gasPrice: gasPrice } )
